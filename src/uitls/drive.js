...
export const uploadToGooglDrive = async (formData) => {
  ...
  const fileMetadata = {
    name: file.name,
    parents: ["1YWv5pAOucfgtIm-PK_t4Muuf4mkQwf8l"],
  };
  ...
  return { viewLink: newFileviewLink, fileId, success: true };
};

export const downloadFromGooglDrive = async (fileId) => {
  const auth = new google.auth.GoogleAuth({
    credentials: {
      type: "service_account",
      project_id: "productive-ielts",
  ...
  const start = 0; // Number(range?.replace(/\D/g, "")) ?? 0; // TOOD: remove type assertion

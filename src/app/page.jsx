...
export default function Home() {
  ...
  async function onSubmit(event) {
    ...
    const res = await uploadToGooglDrive(formData);
    console.log(res);
  }

  const downloadFromGooglDrive = async (fileId, range) => {
  ...
  return (
    <div>
      ...
      <button onClick={() => console.log(downloadFromGooglDrive(fileId))}>
        Download
      </button>
      <VideoPlayer />
    </div>
...
const VideoPlayer = ({ fileId }) => {
  ...
  return (
    <section>
      ...
      <video src="/drive" controls width="600"></video>
    </section>
  );
};
